...
{ 
    ...
    */
    ...
    void LinOpSysSolverMDWFArray::init(Handle< FermState<T,P,Q> > fermstate) 
    {
        ...
        multi1d<int> node_coords(4), node_neighbor_up(4), node_neighbor_down(4);
        int master_p;
        ...
        //set up neighbors:
        for(int mu=0; mu < Nd; mu++) { 
            multi1d<int> tmpcoord=node_coords;
		
            //up-direction:
            tmpcoord[mu]=node_coords[(mu+1)%network[mu]];
            node_neighbor_up[mu]=Layout::nodeNumber(tmpcoord);
		
            //down-direction:
            tmpcoord[mu]=node_coords[(mu-1+network[mu])%network[mu]];
            node_neighbor_down[mu]=Layout::nodeNumber(tmpcoord);
        }
        ...
        config.rank=4;
        config.lat=lattice.slice();
        config.ls=invParam.N5;
        config.net=network.slice();
        config.neighbor_up=node_neighbor_up.slice();
        config.neighbor_down=node_neighbor_down.slice();
