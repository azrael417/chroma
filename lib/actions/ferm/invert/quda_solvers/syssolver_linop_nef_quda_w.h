    ...
    public:
        ...
        */
        ...
        A(A_), invParam(invParam_) 
        {
            ...
            //only asymmetric DWF supported at the moment:
            QDPIO::cout << "Using Asymmetric Linop: A_oo - D_oe A^{-1}_ee D_eo" << std::endl;
            ...
            //load gauge
            StopWatch swatch_loadgauge;
            swatch_loadgauge.reset();
            swatch_loadgauge.start();
            loadGaugeQuda((void *)gauge, &q_gauge_param); 
            swatch_loadgauge.stop();
      
            QDPIO::cout << "Initialization time: " << swatch_init.getTimeInSeconds() << " s." << std::endl;
            QDPIO::cout << "Loading Gauge: " << swatch_loadgauge.getTimeInSeconds() << " s." << std::endl;
        ...
        */
        SystemSolverResults_t operator() (multi1d<T>& psi, const multi1d<T>& chi) const
        {
            ...
            if ( invParam.axialGaugeP ) { 
                ...
                QDPIO::cout << "Gauge Fixing time and transformation: " << swatch_transform.getTimeInSeconds() << " s." << std::endl;
            }
            else { 
            ...
            // Check Solution
            {
                ...
                QDPIO::cout << "QUDA_"<< solver_string <<"_NEF_SOLVER: " << res.n_count << " iterations. Max. Rsd = " << res.resid << " Max. Relative Rsd = " << rel_resid <<  " (check-time=" << swatch_check.getTimeInSeconds() << " s)" << std::endl;
